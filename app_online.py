...
def main():
    ...
    st.caption("AI Legal Assistant powered by Falcon-RW-1B and mpnet embeddings (Online Mode)")
    
    # Show info about online mode
    with st.expander("ℹ️ Online Mode Info"):
        st.info("""
        **Online Mode**: This version downloads models from Hugging Face Hub on first run.
        - Faster setup (no manual downloads needed)
        - Requires internet connection
        - Models are cached for subsequent runs
        - Slightly slower first run due to downloading
        """)
